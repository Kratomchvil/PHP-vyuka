...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $formData['predmet'] = $_POST['predmet'] ?? '';
    $formData['priorita'] = $_POST['priorita'] ?? 'normalni';
    ...
    }elseif($formData['jmeno'] !== '' && mb_strlen($formData['jmeno'])>50){
    ...
    if($formData['predmet'] === ''){
        $chyby['predmet'] = 'Předmět je povinný!';
    }elseif($formData['predmet'] !== ''&& !array_key_exists($formData['predmet'], $predmety)){
        $chyby['predmet'] = 'Neplatný předmět!';
    }

    if($formData['priorita'] !== '' && !array_key_exists($formData['priorita'], $priority)){
        $chyby['priorita'] = 'Neplatná priorita!';
    }

    if($formData['zprava'] === ''){
        $chyby['zprava'] = 'Zpráva je povinná!';
    }elseif($formData['zprava'] !== '' && mb_strlen($formData['zprava']) < 10){
        $chyby['zprava'] = 'Zpráva musí mít alespoň 10 znaků!';
    }elseif($formData['zprava'] !== '' && mb_strlen($formData['zprava']) > 1000){
